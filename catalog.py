...
class Catalog(object):
    ...
    def load_data(self, object_name, filter_name, fname=None):
        if fname is None:
            if os.path.isfile(filter_name):
                fname = filter_name
            else:
                if self.yaml[object_name][filter_name].startswith('https://chandra.harvard.edu'):
                    fname_url = self.yaml[object_name][filter_name]
                    ...
                    fname = os.path.join('data', object_name, self.yaml[object_name][filter_name])
    ...
    def scrape_chandra(self):
        ...
        for match in re.finditer(regex, contents):
            ...
            grp['filter'] = self.convert_filter(grp['filter'])
            if grp['object'] not in self.yaml:
                self.yaml[grp['object']] = {}
                self.remote_objects.append(grp['object'])
            if grp['filter'] not in self.yaml[grp['object']]:
                self.yaml[grp['object']][grp['filter']] = '/'.join(
